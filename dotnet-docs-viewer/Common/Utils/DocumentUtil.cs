...
namespace Common.Utils
{
    public static class DocumentUtil
    {
        ...
        // 1. PDF → PNG 리스트
        // ======================================================
        public static Task<List<byte[]>> ConvertPdfToPng(byte[] pdfBytes, float scale = 2.0f)
        {
            return Task.Run(() =>
            {
        ...
        // 2. PPTX → PNG 리스트
        // ======================================================
        public static Task<List<byte[]>> ConvertPptxToPng(byte[] pptxBytes, int slideWidth = 1280, int slideHeight = 720)
        {
            return Task.Run(() =>
            {
        ...
        // 3. XLSX → PNG 리스트
        // ======================================================
        public static Task<List<byte[]>> ConvertXlsxToPng(byte[] xlsxBytes, int cellWidth = 140, int cellHeight = 40)
        {
            return Task.Run(() =>
            {
                var result = new List<byte[]>();
        ...
        public static byte[] MergeImagesVertically(List<byte[]> imageBytesList)
        {
            var bitmaps = new List<SKBitmap>();
            int totalHeight = 0;
            int maxWidth = 0;

            // Load images and compute total size
            foreach (var bytes in imageBytesList)
            {
                ...
                maxWidth = System.Math.Max(maxWidth, bitmap.Width);
            }

            // Create final merged image
            using var surface = SKSurface.Create(new SKImageInfo(maxWidth, totalHeight));
            ...
            {
                canvas.DrawBitmap(bmp, new SKPoint(0, yOffset));
                yOffset += bmp.Height;
            }
